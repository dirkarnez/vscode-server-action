...
const progress_js_1 = __nccwpck_require__(5659);
const request = __importStar(__nccwpck_require__(729));
const util_2 = __nccwpck_require__(2654);
...
async function downloadVSCodeArchive(options) {
    ...
    return {
        ...
        length: totalBytes,
        commit: res.headers['x-source-commit'],
    };
}
/**
    ...
    if (format === 'zip') {
        ...
        try {
            ...
            if (process.platform === 'win32') {
                const [buffer, JSZip] = await Promise.all([(0, util_2.streamToBuffer)(stream), __nccwpck_require__.e(/* import() */ 592).then(__nccwpck_require__.t.bind(__nccwpck_require__, 3592, 23))]);
                await checksum;
...
async function download(options = {}) {
    ...
    if (fs.existsSync(path.join(downloadedPath, COMPLETE_FILE_NAME))) {
        if (version.isInsiders) {
            ...
            const { version: currentHash, date: currentDate } = (0, util_2.insidersDownloadDirMetadata)(downloadedPath, platform, reporter, latestHash);
    ...
    for (let i = 0;; i++) {
        try {
            ...
            reporter.report({
                stage: progress_js_1.ProgressReportStage.NewInstallComplete,
                downloadedPath,
...
var download_1 = __nccwpck_require__(9509);
...
var runTest_1 = __nccwpck_require__(1080);
...
var util_1 = __nccwpck_require__(2654);
...
__exportStar(__nccwpck_require__(5659), exports);
...
const makeConsoleReporter = async () => {
    ...
    const { default: ora } = await __nccwpck_require__.e(/* import() */ 960).then(__nccwpck_require__.bind(__nccwpck_require__, 3960));
    let version;
...
const https = __importStar(__nccwpck_require__(5687));
const util_1 = __nccwpck_require__(2654);
...
const cp = __importStar(__nccwpck_require__(2081));
const download_1 = __nccwpck_require__(9509);
const util_1 = __nccwpck_require__(2654);
/**
...
async function runTests(options) {
    ...
    return innerRunTests(options.vscodeExecutablePath, args, options.stdout ?? process.stdout, options.stderr ?? process.stderr, options.extensionTestsEnv);
}
const SIGINT = 'SIGINT';
async function innerRunTests(executable, args, stdout, stderr, testRunnerEnv) {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.VSCodeCommandError = exports.Version = exports.isRunnableOnHost = exports.isPlatformCLI = exports.isPlatformServer = exports.systemDefaultPlatform = void 0;
...
const getPlatformArch = (platform) => {
    if (platform.includes('arm64'))
        return 'arm64';
    if (platform.includes('armhf'))
        return 'arm';
...
const isRunnableOnHost = (platform) => {
    const osMatches = process.platform === 'win32'
...
exports.isRunnableOnHost = isRunnableOnHost;
 ...
 */
function resolveDarwinAppExecutable(appPath) {
    ...
    try {
        const plist = (0, fs_1.readFileSync)(infoPlistPath, 'utf-8');
        const match = plist.match(/<key>CFBundleExecutable<\/key>\s*<string>([^<]+)<\/string>/);
        if (match) {
            const candidate = path.resolve(macosDir, match[1]);
            if (candidate.startsWith(macosDir + path.sep) && (0, fs_1.existsSync)(candidate)) {
                return candidate;
            }
        }
    }
    ...
    try {
        const files = (0, fs_1.readdirSync)(macosDir, { withFileTypes: true }).filter((e) => e.isFile());
        if (files.length === 1) {
...
function insidersDownloadDirMetadata(dir, platform, reporter, latestHash) {
    let productJsonPath;
    if ((0, exports.isPlatformServer)(platform)) {
 ...
 */
function findWindowsProductJsonPath(dir, latestHash) {
...
/* harmony import */ var execa__WEBPACK_IMPORTED_MODULE_3__ = __nccwpck_require__(16);
