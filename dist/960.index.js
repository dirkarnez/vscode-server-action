...
const spinners = Object.assign({}, __webpack_require__(6374)); // eslint-disable-line import/extensions
...
__webpack_require__.d(__webpack_exports__, {
  ...
  "spinners": () => (/* reexport */ cli_spinners)
});

// EXTERNAL MODULE: external "node:process"
var external_node_process_ = __webpack_require__(7742);
;// CONCATENATED MODULE: ./node_modules/ora/node_modules/chalk/source/vendor/ansi-styles/index.js
...
;// CONCATENATED MODULE: ./node_modules/ora/node_modules/chalk/source/vendor/supports-color/index.js
...
var cli_spinners = __webpack_require__(2031);
;// CONCATENATED MODULE: ./node_modules/log-symbols/node_modules/chalk/source/vendor/ansi-styles/index.js
...
const ansi_styles_modifierNames = Object.keys(ansi_styles_styles.modifier);
...
const ansi_styles_colorNames = [...ansi_styles_foregroundColorNames, ...ansi_styles_backgroundColorNames];
...
/* harmony default export */ const vendor_ansi_styles = (ansi_styles_ansiStyles);

;// CONCATENATED MODULE: ./node_modules/log-symbols/node_modules/chalk/source/vendor/supports-color/index.js
...
// From: https://github.com/sindresorhus/has-flag/blob/main/index.js
/// function hasFlag(flag, argv = globalThis.Deno?.args ?? process.argv) {
function supports_color_hasFlag(flag, argv = globalThis.Deno ? globalThis.Deno.args : external_node_process_.argv) {
	const prefix = flag.startsWith('-') ? '' : (flag.length === 1 ? '-' : '--');
	const position = argv.indexOf(prefix + flag);
	const terminatorPosition = argv.indexOf('--');
	return position !== -1 && (terminatorPosition === -1 || position < terminatorPosition);
...
let supports_color_flagForceColor;
if (
	supports_color_hasFlag('no-color')
	|| supports_color_hasFlag('no-colors')
	|| supports_color_hasFlag('color=false')
	|| supports_color_hasFlag('color=never')
) {
	supports_color_flagForceColor = 0;
} else if (
	supports_color_hasFlag('color')
	|| supports_color_hasFlag('colors')
	|| supports_color_hasFlag('color=true')
	|| supports_color_hasFlag('color=always')
) {
	supports_color_flagForceColor = 1;
}

function supports_color_envForceColor() {
	if ('FORCE_COLOR' in supports_color_env) {
		if (supports_color_env.FORCE_COLOR === 'true') {
			return 1;
		}

		if (supports_color_env.FORCE_COLOR === 'false') {
			return 0;
		}

		return supports_color_env.FORCE_COLOR.length === 0 ? 1 : Math.min(Number.parseInt(supports_color_env.FORCE_COLOR, 10), 3);
	}
}

function supports_color_translateLevel(level) {
	if (level === 0) {
		return false;
	}
...
function supports_color_supportsColor(haveStream, {streamIsTTY, sniffFlags = true} = {}) {
	const noFlagForceColor = supports_color_envForceColor();
	if (noFlagForceColor !== undefined) {
		supports_color_flagForceColor = noFlagForceColor;
	}

	const forceColor = sniffFlags ? supports_color_flagForceColor : noFlagForceColor;
	...
	if (sniffFlags) {
		if (supports_color_hasFlag('color=16m')
			|| supports_color_hasFlag('color=full')
			|| supports_color_hasFlag('color=truecolor')) {
			return 3;
		}

		if (supports_color_hasFlag('color=256')) {
			return 2;
		}
	}
	...
	// Check for Azure DevOps pipelines.
	// Has to be above the `!streamIsTTY` check.
	if ('TF_BUILD' in supports_color_env && 'AGENT_NAME' in supports_color_env) {
		return 1;
	}
	...
	if (supports_color_env.TERM === 'dumb') {
		return min;
	}
	...
	if ('CI' in supports_color_env) {
		if (['GITHUB_ACTIONS', 'GITEA_ACTIONS', 'CIRCLECI'].some(key => key in supports_color_env)) {
			return 3;
		}

		if (['TRAVIS', 'APPVEYOR', 'GITLAB_CI', 'BUILDKITE', 'DRONE'].some(sign => sign in supports_color_env) || supports_color_env.CI_NAME === 'codeship') {
			return 1;
		}
	...
	if ('TEAMCITY_VERSION' in supports_color_env) {
		return /^(9\.(0*[1-9]\d*)\.|\d{2,}\.)/.test(supports_color_env.TEAMCITY_VERSION) ? 1 : 0;
	}

	if (supports_color_env.COLORTERM === 'truecolor') {
		return 3;
	}

	if (supports_color_env.TERM === 'xterm-kitty') {
		return 3;
	}

	if (supports_color_env.TERM === 'xterm-ghostty') {
		return 3;
	}

	if (supports_color_env.TERM === 'wezterm') {
		return 3;
	}
	...
	if (/-256(color)?$/i.test(supports_color_env.TERM)) {
		return 2;
	}

	if (/^screen|^xterm|^vt100|^vt220|^rxvt|color|ansi|cygwin|linux/i.test(supports_color_env.TERM)) {
		return 1;
	}

	if ('COLORTERM' in supports_color_env) {
		return 1;
	}
...
function supports_color_createSupportsColor(stream, options = {}) {
	const level = supports_color_supportsColor(stream, {
		streamIsTTY: stream && stream.isTTY,
		...options,
	});

	return supports_color_translateLevel(level);
}

const vendor_supports_color_supportsColor = {
	stdout: supports_color_createSupportsColor({isTTY: external_node_tty_.isatty(1)}),
	stderr: supports_color_createSupportsColor({isTTY: external_node_tty_.isatty(2)}),
};

/* harmony default export */ const vendor_supports_color = (vendor_supports_color_supportsColor);
...
const {stdout: source_stdoutColor, stderr: source_stderrColor} = vendor_supports_color;
...
const chalk_source_styles = Object.create(null);
...
for (const [styleName, style] of Object.entries(vendor_ansi_styles)) {
	chalk_source_styles[styleName] = {
		get() {
...
const source_getModelAnsi = (model, level, type, ...arguments_) => {
	if (model === 'rgb') {
		if (level === 'ansi16m') {
			return vendor_ansi_styles[type].ansi16m(...arguments_);
		}

		if (level === 'ansi256') {
			return vendor_ansi_styles[type].ansi256(vendor_ansi_styles.rgbToAnsi256(...arguments_));
		}

		return vendor_ansi_styles[type].ansi(vendor_ansi_styles.rgbToAnsi(...arguments_));
	}

	if (model === 'hex') {
		return source_getModelAnsi('rgb', level, type, ...vendor_ansi_styles.hexToRgb(...arguments_));
	}

	return vendor_ansi_styles[type][model](...arguments_);
};

const source_usedModels = ['rgb', 'hex', 'ansi256'];

for (const model of source_usedModels) {
	chalk_source_styles[model] = {
		get() {
			const {level} = this;
			return function (...arguments_) {
				const styler = source_createStyler(source_getModelAnsi(model, source_levelMapping[level], 'color', ...arguments_), vendor_ansi_styles.color.close, this[source_STYLER]);
	...
	const bgModel = 'bg' + model[0].toUpperCase() + model.slice(1);
	chalk_source_styles[bgModel] = {
		get() {
			const {level} = this;
			return function (...arguments_) {
				const styler = source_createStyler(source_getModelAnsi(model, source_levelMapping[level], 'bgColor', ...arguments_), vendor_ansi_styles.bgColor.close, this[source_STYLER]);
				return source_createBuilder(this, styler, this[source_IS_EMPTY]);
			};
		},
	};
...
Object.defineProperties(source_createChalk.prototype, chalk_source_styles);
...
var emoji_regex = __webpack_require__(4128);
